...
exports.default = function (request, response, next) {
  ...
  if (!token) {
    return next(new _httpErrors2.default(401, 'AUTH - bad token'));
  }

  return promisify(_jsonwebtoken2.default.verify)(token, process.env.PENGUIN_PIC_SECRET).catch(function (error) {
    Promise.reject(new _httpErrors2.default(400, 'AUTH - jsonWebToken Error ' + error));
